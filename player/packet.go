...
func (p *Player) handlePacketQueue() {
	...
	p.TryDebug(fmt.Sprint("bufferSize=", startLen, " credits=", p.queueCredits, " ignored=", ignored, " doDouble=", doDouble, " filling=", buffer == nil), DebugTypeChat, p.debugger.UsePacketBuffer)
	if buffer == nil {
		if !p.isBufferReady {
			return
		}
