...
func (ack *ChunkUpdate) Run() {
	...
	cInfo, err := oworld.CacheChunk(ack.pk, ack.mPlayer.GameDat.UseBlockNetworkIDHashes)
	if err != nil {
...
func (ack *SubChunkUpdate) Run() {
	...
	for _, entry := range ack.pk.SubChunkEntries {
		...
		case protocol.SubChunkResultSuccess:
			...
			cachedSub, err := oworld.CacheSubChunk(buf, ch, chunkPos, ack.mPlayer.GameDat.UseBlockNetworkIDHashes)
			if err != nil {
				ack.mPlayer.Disconnect(fmt.Sprintf(game.ErrorInternalDecodeChunk, err))
				continue
			}
			ch.Sub()[cachedSub.Layer()] = cachedSub.SubChunk()
			ack.mPlayer.World().AddSubChunk(chunkPos, cachedSub.Hash(), ack.mPlayer.GameDat.UseBlockNetworkIDHashes)
