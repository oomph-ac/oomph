...
func BlockCollisions(b world.Block, pos df_cube.Pos, src world.BlockSource) []df_cube.BBox32 {
	...
	case 0:
		return nil
	case 1:
		return []df_cube.BBox32{game.BBox32FromBBox(dfBoxes[0])}
	}

	boxes := make([]df_cube.BBox32, len(dfBoxes))
...
func NearbyBlockCollisions(aabb df_cube.BBox32, src world.BlockSource) iter.Seq[BlockSearchResult] {
...
func NearbyBlocks(aabb df_cube.BBox32, includeAir bool, includeUnknown bool, src world.BlockSource) iter.Seq[BlockSearchResult] {
	return func(yield func(BlockSearchResult) bool) {
		min, max := aabb.Min(), aabb.Max()
		minX, minY, minZ := int(math32.Floor(min[0])), int(math32.Floor(min[1])), int(math32.Floor(min[2]))
		maxX, maxY, maxZ := int(math32.Ceil(max[0])), int(math32.Ceil(max[1])), int(math32.Ceil(max[2]))
...
func HasNearbyBBoxes(aabb df_cube.BBox32, src world.BlockSource) bool {
...
func NearbyBBoxes(aabb df_cube.BBox32, src world.BlockSource) []df_cube.BBox32 {
	bboxList, _ := scanNearbyBBoxes(aabb, src, false)
	return bboxList
}

func scanNearbyBBoxes(aabb df_cube.BBox32, src world.BlockSource, firstOnly bool) ([]df_cube.BBox32, bool) {
	grown := aabb.Grow(1.0)
	min, max := grown.Min(), grown.Max()
	minX, minY, minZ := int(math32.Floor(min[0])), int(math32.Floor(min[1])), int(math32.Floor(min[2]))
	maxX, maxY, maxZ := int(math32.Ceil(max[0])), int(math32.Ceil(max[1])), int(math32.Ceil(max[2]))

	var bboxList []df_cube.BBox32
		...
		for z := minZ; z <= maxZ; z++ {
			for y := minY; y <= maxY; y++ {
				...
				for _, box := range BlockCollisions(block, pos, src) {
					...
					if bboxList == nil {
						bboxList = make([]df_cube.BBox32, 0, maxIters)
					}
...
func BlockToCubePos(p [3]int32) df_cube.Pos {
	return df_cube.Pos{int(p[0]), int(p[1]), int(p[2])}
}
