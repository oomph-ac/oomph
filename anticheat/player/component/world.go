...
func (c *WorldUpdaterComponent) HandleUpdateBlock(pk *packet.UpdateBlock) {
	...
	blockRuntimeID := c.mPlayer.DecodeBlockRuntimeID(pk.NewBlockRuntimeID)
	if pk.Layer == 1 {
		c.batchedExtraBlockUpdates.SetBlock(pos, blockRuntimeID)
		return
	}
	c.AddPendingUpdate(pos, blockRuntimeID)
}
...
func (c *WorldUpdaterComponent) HandleUpdateSubChunkBlocks(pk *packet.UpdateSubChunkBlocks) {
	...
	for _, entry := range pk.Extra {
		c.batchedExtraBlockUpdates.SetBlock(df_cube.Pos{int(entry.BlockPos.X()), int(entry.BlockPos.Y()), int(entry.BlockPos.Z())}, c.mPlayer.DecodeBlockRuntimeID(entry.BlockRuntimeID))
	}
}
